...
async fn test_phase15_1_real_git_repository_and_architecture_discovery() {
    ...
    tokio::fs::create_dir_all(repo_dir.join("src"))
        .await
        .unwrap();
    tokio::fs::create_dir_all(repo_dir.join("crates/core"))
        .await
        .unwrap();
    tokio::fs::write(
    ...
    assert!(context
        .detected_ecosystems
        .iter()
        .any(|e| e.contains("Rust") || e.contains("Cargo")));
...
async fn test_phase15_3_human_review_approval_and_overlap_detection() {
    ...
    let overlaps = OverlapDetector::detect_and_persist_for_project(&pool, proj.id)
        .await
        .unwrap();
    assert!(
        !overlaps.is_empty(),
        "Should detect overlap on crates/db/schema.rs"
    );

    // Human review: acknowledge overlap warning and approve tasks
    for w in &overlaps {
        OverlapWarningRepository::acknowledge(&pool, w.id)
            .await
            .unwrap();
    }
    TaskRepository::update_status(&pool, task1.id, TaskStatus::Approved)
        .await
        .unwrap();
    TaskRepository::update_status(&pool, task2.id, TaskStatus::Approved)
        .await
        .unwrap();
    ...
    let t1 = TaskRepository::find_by_id(&pool, task1.id)
        ...
        .unwrap()
        .unwrap();
    let t2 = TaskRepository::find_by_id(&pool, task2.id)
        .await
        .unwrap()
        .unwrap();
...
async fn test_phase15_4_multi_agent_registration_and_secure_fleet() {
    ...
    let resp_a = RegistrationHandler::process_registration(&pool, msg_a)
        .await
        .unwrap();
    assert!(
        matches!(resp_a, CoordinatorMessage::RegisterResponse { status, .. } if status == "ok")
    );
    ...
    let resp_b = RegistrationHandler::process_registration(&pool, msg_b)
        .await
        .unwrap();
    assert!(
        matches!(resp_b, CoordinatorMessage::RegisterResponse { status, .. } if status == "ok")
    );

    let agent_a = AgentRepository::find_by_id(&pool, agent_a_id)
        .await
        .unwrap()
        .unwrap();
    let agent_b = AgentRepository::find_by_id(&pool, agent_b_id)
        .await
        .unwrap()
        .unwrap();
...
async fn test_phase15_6_dependency_enforcement_order() {
    ...
    CommandHandler::execute_approve_task(&pool, task_a.id, "Supervisor")
        .await
        .unwrap();
    CommandHandler::execute_approve_task(&pool, task_b.id, "Supervisor")
        .await
        .unwrap();
    ...
    let t_b = TaskRepository::find_by_id(&pool, task_b.id)
        .await
        .unwrap()
        .unwrap();
    assert_eq!(
...
async fn test_phase15_7_agent_crash_recovery_and_reassignment() {
    ...
    TaskRepository::update_status(&pool, task.id, TaskStatus::Approved)
        .await
        .unwrap();
    TaskRepository::assign_agent(&pool, task.id, Some(agent_crashed.id))
        .await
        .unwrap();
    ...
    TaskRepository::assign_agent(&pool, task.id, Some(agent_backup.id))
        .await
        .unwrap();
    TaskRepository::update_status(&pool, task.id, TaskStatus::Executing)
        .await
...
async fn test_phase15_8_cross_agent_git_conflict_and_failure_diagnostics() {
    ...
    assert!(diag
        .remediation_advice
        .iter()
        .any(|r| r.title == "Resolve Cross-Agent Git Merge Conflicts"));
}
...
async fn test_phase15_10_complete_project_lifecycle_execution() {
    ...
    // 5. Human approval
    TaskRepository::update_status(&pool, task_1.id, TaskStatus::HumanReview)
        .await
        .unwrap();
    TaskRepository::update_status(&pool, task_2.id, TaskStatus::HumanReview)
        .await
        .unwrap();
    TaskRepository::assign_agent(&pool, task_1.id, Some(agent_1.id))
        .await
    ...
    CommandHandler::execute_approve_task(&pool, task_1.id, "Lead")
        .await
        .unwrap();
    CommandHandler::execute_approve_task(&pool, task_2.id, "Lead")
        .await
        .unwrap();
    ...
    assert_eq!(
        assigned.len(),
        2,
        "Both approved independent tasks should be assigned concurrently"
    );

    // 7. Complete tasks and verify project timeline
    TaskRepository::update_status(&pool, task_1.id, TaskStatus::Completed)
        .await
        .unwrap();
    TaskRepository::update_status(&pool, task_2.id, TaskStatus::Completed)
        .await
        .unwrap();

    let task_tl = TimelineService::build_task_timeline(&pool, task_1.id)
        .await
        .unwrap();
    assert_eq!(task_tl.status, TaskStatus::Completed);
    assert!(!task_tl.events.is_empty());

    let agent_tl = TimelineService::build_agent_timeline(&pool, agent_1.id)
        .await
        .unwrap();
